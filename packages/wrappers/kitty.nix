...
}:
{
  ...
  env.paths."KITTY_CONFIG_DIRECTORY" = {
    "kitty.conf".text = /* kitty */ ''
      ...
      tab_separator             ""
      tab_bar_min_tabs          1
      tab_title_template        "{fmt.bg._89b4fa}{fmt.fg._1e1e2e} {index} {fmt.bg.tab}{fmt.fg.tab} {tab.active_exe:<3.20} "
      active_tab_title_template "{fmt.bg._fab387}{fmt.fg._1e1e2e} {index} {fmt.bg.tab}{fmt.fg.tab} {('/'.join(tab.active_wd.split('/')[-2:]) if tab.active_wd else title):<3.20} "
