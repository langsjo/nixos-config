...
in
{
  options.custom.gpg.enable = lib.mkEnableOption "gpg" // {
    default = true;
  };
  config = lib.mkIf cfg.enable {
    ...
    environment.etc = {
      "gnupg/gpg.conf".source = settingsFormat.generate "gpg.conf" {
      ...
      "gnupg/scdaemon.conf".source = settingsFormat.generate "scdaemon.conf" {
        disable-ccid = "";
      };
    };
  };
